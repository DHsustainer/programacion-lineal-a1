...
const SolarPanelOptimizer = () => {
	...
	const optimize = () => {
		...
		for (
			let standard = 0;
			standard <=
			Math.floor(constraints.materialLimit / panelTypes.standard.materialUsage);
			standard++
		) {
			for (
				let premium = 0;
				premium <=
				Math.floor(
					constraints.materialLimit / panelTypes.premium.materialUsage
				);
				premium++
			) {
				const materialUsed =
					standard * panelTypes.standard.materialUsage +
					premium * panelTypes.premium.materialUsage
				const laborUsed =
					standard * panelTypes.standard.laborHours +
					premium * panelTypes.premium.laborHours
				const emissions =
					standard * panelTypes.standard.emissions +
					premium * panelTypes.premium.emissions

				if (
					materialUsed <= constraints.materialLimit &&
					laborUsed <= constraints.laborHours &&
					emissions <= constraints.maxEmissions
				) {
					const profit =
						standard * panelTypes.standard.profit +
						premium * panelTypes.premium.profit
		...
		setResults({
			...
			profit: maxProfit,
			environmentalImpact:
				optimalStandard * panelTypes.standard.emissions +
				optimalPremium * panelTypes.premium.emissions,
		...
		<div className='w-full mx-auto p-4'>
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Optimize la producción considerando restricciones de materiales,
						trabajo y emisiones de CO2.
					</CardDescription>
				</CardHeader>
				<CardContent className='space-y-6'>
					...
					<div className='space-y-4'>
						...
						<div className='grid grid-cols-3 gap-4'>
							...
							<div className='space-y-2'>
								<Label htmlFor='maxEmissions'>
									Límite de Emisiones (ton CO2)
								</Label>
								<Input
									id='maxEmissions'
									name='maxEmissions'
								...
								<AlertDescription>
									<div className='grid grid-cols-2 gap-4 mt-2'>
										...
										<div>
											...
											<p>
												Impacto Ambiental:{' '}
												{results.environmentalImpact.toFixed(2)} ton CO2
											</p>
										</div>
									</div>
								</AlertDescription>
					...
					<div className='text-sm text-gray-500'>
						Los resultados se basan en las restricciones ingresadas y los
						parámetros predefinidos para cada tipo de panel.
					</div>
				</CardContent>
			</Card>
		</div>
